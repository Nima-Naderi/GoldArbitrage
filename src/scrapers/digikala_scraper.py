...
class DigikalaScraper(BaseScraper):
    """Scraper for digikala.com digital gold"""
    
    def __init__(self):
        super().__init__(
            url="https://www.digikala.com/wealth/landing/digital-gold",
            name="digikala",
            use_selenium=True
        )
    
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Digikala website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .wealth-price, .digital-gold-price',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .wealth-change, .percentage',
            'price_container': '.price-container, .wealth-data, .gold-widget, .digital-gold-info',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Digikala"""
        try:
