...
class TalappScraper(BaseScraper):
    """Scraper for talapp.ir"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Talapp website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .talapp-price, .app-price',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .talapp-change',
            'price_container': '.price-container, .talapp-data, .app-data',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Talapp"""
        try:
