...
class DaricScraper(BaseScraper):
    """Scraper for daric.gold"""
    
    def __init__(self):
        super().__init__(
            url="https://daric.gold/",
            name="daric",
            use_selenium=True
        )
    
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Daric website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .daric-price, .current-price',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .daric-change',
            'price_container': '.price-container, .daric-data, .gold-info',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Daric"""
        try:
