...
class TechnogoldScraper(BaseScraper):
    """Scraper for technogold.gold"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Technogold website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .techno-price, .live-rate',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .techno-change',
            'price_container': '.price-container, .techno-data, .gold-widget',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Technogold"""
        try:
