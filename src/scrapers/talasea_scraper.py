...
class TalaseaScraper(BaseScraper):
    """Scraper for talasea.ir"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Talasea website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"]',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"]',
            'price_container': '.price-container, .gold-data, .price-box',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Talasea"""
        try:
