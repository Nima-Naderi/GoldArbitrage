...
class MelligoldScraper(BaseScraper):
    """Scraper for melligold.com"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Melligold website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .live-price, .current-rate',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .percentage, .daily-change',
            'price_container': '.price-container, .gold-data, .rate-box, .price-widget',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Melligold"""
        try:
