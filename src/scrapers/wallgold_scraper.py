...
class WallgoldScraper(BaseScraper):
    """Scraper for wallgold.ir"""
    ...
    def get_selectors(self) -> Dict[str, str]:
        """Return CSS selectors for Wallgold website elements"""
        return {
            'price': '.price, .gold-price, [class*="price"], [class*="قیمت"], .wall-price',
            'change': '.change, .price-change, [class*="change"], [class*="تغییر"], .wall-change',
            'price_container': '.price-container, .wall-data, .gold-box',
            'currency': 'span:contains("ریال"), span:contains("تومان")'
        }
    
    def scrape(self) -> Optional[GoldPriceData]:
        """Scrape gold price data from Wallgold"""
        try:
